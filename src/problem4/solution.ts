...
// Additionally, I thinking of using Array.from to create an array in a specific range, So the test cases can be more dynamic and also we could also understand
// the theory as sum number in array from 1 to n.
// However to keep the input as the requirement, I will leave the method interface as `sum_to_n(n: number): number`, and handle the Range create in another function.
...
const arrayRange = ({
    ...
    stop = defaultStop,
    step = defaultStep
}: { start?: number; stop?: number; step?: number }): Array<number> =>
    Array.from(
...
const sum_to_n_a = (n: number): number => {
    ...
    // Use this if you want the best performance and you are sure that the input will always be a simple range from 1 to n.
    return result
}
